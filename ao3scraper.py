...
def scrape_urls():
    ...
    for item in track(fic_table, description="Fetching data from AO3..."):
        ...
        if "title" and "chapters" and "updated" not in item:
            table.add_row(item_index, "[link=" + item[URL_POS] + "]" + web_tags[0]+ "[/link]", web_tags[1], web_tags[2])
        else:
        ...
        #print("UPDATE fics SET title = '" + web_tags[0] + "', chapters = '" + web_tags[1] + "', updated = '" + web_tags[2] + "' WHERE url = '" + target_entry[URL_POS] + "';")
        cursor.execute("UPDATE fics SET title = \"" + web_tags[0] + "\", chapters = \"" + web_tags[1] + "\", updated = \"" + web_tags[2] + "\" WHERE url = \"" + target_entry[URL_POS] + "\";")
        connection.commit()
...
def get_tags(url, item):
    ...
    if not page.ok:
        print(page.status_code, "Error!", style="red")
...
def construct_rich_table():
    ...
    for item in fic_table:
            ...
            if item[TITLE_POS] == None:
                table.add_row(item_index, "[link=" + item[URL_POS] + "]FIC DATA NOT YET SCRAPED[/link]", item[CHAPTER_POS], item[LAST_UPDATED_POS])
            else:
                # table.add_row(str(fic_table.index(item) + 1) + ". [link=" + item[1] + "]" + item_id + "[/link]", item[0], item[2], item[3])
                table.add_row(item_index, "[link=" + item[URL_POS] + "]" + item[TITLE_POS] + "[/link]", item[CHAPTER_POS], item[LAST_UPDATED_POS])
